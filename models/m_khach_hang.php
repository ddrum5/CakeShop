...
class M_khach_hang extends database
{
  ...
  public function Sua_khach_hang($ma_kh, $ten_kh, $phai, $email, $dia_chi, $dien_thoai, $mat_khau_moi, $mat_khau_cu)
  {
    if ($mat_khau_moi == "") {
      $mat_khau = $mat_khau_cu;
    }
    else {
      $mat_khau = md5($mat_khau_moi);
    }


    $sql = "UPDATE khach_hang 
    set ten_khach_hang=?,phai=?, email=?,dia_chi=?,dien_thoai=?, matKhau=?
    where ma_khach_hang=?";
    $this->setQuery($sql);
    $param = array($ten_kh, $phai, $email, $dia_chi, $dien_thoai, $mat_khau, $ma_kh);
    $this->execute($param);
