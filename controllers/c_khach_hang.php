...
class C_khach_hang
{
    ...
    public function CapNhapSoLuongHoa()
    {
        ...
        $m_banh = new M_banh();
        foreach ($_SESSION["giohang"] as $k => $value) {
            $sl_hoa_db = $m_banh->doc_banh_theo_ma($k)->SoLuongSP;
            $sl = $sl_hoa_db - $value;
            $m_banh->CapNhapSoLuongHoa($sl, $k);
        }
    }
    public function GuiMail($ma_hoa_don)
    {
        ...
        $mail->Port = 465;
        $mail->SMTPSecure = "ssl";
        $mail->Username = 'phamxuandinh1105@gmail.com'; // SMTP username
        $mail->Password = 'zbepkzirsopkmbiw'; // SMTP password
        $mail->setFrom('phamxuandinh1105@gmail.com', 'Bánh ngọt An An');
        $mail->addAddress($_SESSION['email']); // Add a recipient
        $mail->Subject = "Cám ơn bạn đã mua bánh tại Bánh ngọt An An";
        $noidung = "<h3 style='font-size:13px;font-weight:bold;color:#02acea;text-transform:uppercase;margin:20px 0 5px 0'>
        Thông tin đơn hàng mã $ma_hoa_don</h3>";
        $noidung .= "<div style='margin:0 0 5px 0'><b>Thông tin thanh toán</b></div><div style='margin:0 0 5px 0'>" .
            $_SESSION['hoTen'] . "</div><div style='margin:0 0 5px 0'>" . $_SESSION['dia_chi'] . "</div><div style='margin:0 0 5px 0'>" . $_SESSION['dien_thoai'] . "</div>";
        ...
        $noidung.= "Bạn có thể kiểm tra đơn hàng <a href='http://localhost/banhngotanan/check_hoa_don.php'>tại đây</a>";
