...
class M_banh extends database
{
    function doc_tat_ca_banh(){
    ...
    function update_banh($tenHoa,$tenHoaURL,$gia,$giakm,$thanhPhan,$noiDung,$hinh,$maLoai,$sl,$maHoa){
    ...
    function doc_banh_theo_ma($maHoa){
        $sql = "SELECT * FROM hoa WHERE MaHoa=?";
        $this->setQuery($sql);
        return $this->loadRow(array($maHoa));
    }
    public function them_banh($tenHoa,$tenHoa_URL, $gia,$gia_khuyen_mai, $thanhPhan, $noiDung, $hinh, $maLoai,$sl){
